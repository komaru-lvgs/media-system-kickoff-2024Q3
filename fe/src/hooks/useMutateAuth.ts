...
import { Credential, Member, Team } from '../types'
...
export const useMutateAuth = () => {
  ...
  const registerTeamMutation = useMutation(
    async (team: Team) => {
      await axios.post(`${process.env.REACT_APP_API_URL}/register/team`, team)
    },
    {
      onError: (err: any) => {
        if (err.response.data.message) {
  ...
  const registerMemberMutation = useMutation(
    async (members: Member[]) => {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/register/member`,
        members,
      )
    },
    {
